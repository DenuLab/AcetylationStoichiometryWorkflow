...
data <- read.csv("./Serum_starve_TC/SSMCF7_Spectronaut_analysis/20181031_SSMCF7_stoich_Spectronaut10_reprocess_correctFASTA_1sthalf.csv")

# Spectral library with isotope labels
spec_lib_isotope <- read.csv("./MCF7_Spectral_Libraries/20181031_tidy_mcf7_spec_lib_spectronaut10_correctFASTA_isotope.csv")
...
# Time point 
data$time_point <- as.numeric(unlist(lapply(data$R.FileName, function(x){
...
data <- data %>% 
  arrange(EG.StrippedSequence, fragment_ion_sequence, time_point)
...
data_1k <- data_1k %>% 
  select(sample_id, time_point, fraction, PG.Genes, PG.ProteinDescriptions, PG.ProteinGroups, 
...
# Removing duplicate rows
data_1k <- data_1k %>% 
  distinct(sample_id, time_point, fraction, PG.Genes, PG.ProteinDescriptions, PG.ProteinGroups, 
...
data_1k <- data_1k %>% 
  select(sample_id, time_point, fraction, PG.ProteinGroups, PG.ProteinDescriptions, PG.Genes, 
...
frg_sum <- frg_stoich %>%
  group_by(sample_id, time_point, PG.ProteinGroups, PG.ProteinDescriptions, 
...
# write.table(frg_sum, file = "./Serum_Stimulated_MCF7/Cleaned/20181101_SS_tc_tidy_frag_stoich_output_REPROCESS_1sthalf.csv", sep = ",", row.names = FALSE)
# write.table(pep_sum, file = "./Serum_Stimulated_MCF7/Cleaned/20181101_SS_tc_tidy_peptide_stoich_output_REPROCESS_1sthalf.csv", sep = ",", row.names = FALSE)
